...
class ScanController(QObject):
    ...
    statusFrame  = Signal(int, int, int, int, int, int)  # passthrough 'S' frames
    #   (tm, adc, dac_z, bias, steps, flags) — STRM keeps pushing during RUN,
    #   so the session recording continues through a scan via this signal.
    rawBlock     = Signal(int, int, int, object)  # passthrough 'R' blocks so a
    #   raw capture survives the port handoff to the scan reader.
    ...
    # Firmware scan positions are in 20-bit sigma-delta space; the DAC is
    # 16-bit, so firmware units = DAC LSB x 16.  Sending 16-bit units here
    # made every continuous scan sweep 1/16 of the commanded size — THE
    # root cause of "continuous never finds the morphology" (2026-07-15).
    _POSITION_PER_DAC_LSB = 16   # 2^(POSITION_BITS-20 minus DAC_BITS-16)

    def _nm_to_xy_lsb_span(self, nm: float) -> int:
        """Convert an XY *span/offset* in nm to firmware 20-bit position
        units (DAC LSB x 16).

        This is a delta (no midpoint offset): the firmware adds scanSize/xo
        directly into the scan-counter projection, then sigma-delta divides
        by 16 on the way to the 16-bit DAC.
        """
    ...
    def _ensure_reader(self) -> bool:
        ...
        if self._reader is not None:
            ...
            try:
                ...
                self._reader.statusFrame.disconnect()
                self._reader.rawBlock.disconnect()
        ...
        # Signal-to-signal: re-emitted on the reader thread, so a
        # DirectConnection consumer (the status logger tap) stays off the
        # GUI thread.
        self._reader.statusFrame.connect(self.statusFrame)
        self._reader.rawBlock.connect(self.rawBlock)
        self._reader.start()
    ...
    @Slot(int)
    def set_samples_per_pixel(self, n: int):
