...
class PlotFrame(QtWidgets.QWidget):
    ...
    def update_histogram(self, centers, counts, current_index=None):
        ...
        if len(centers) == 0:
            # Explicit clear: zero the bars and hide the arrow.  Returning
            # early here silently kept stale bars on screen after the
            # Stability tab's Clear (bench 2026-07-15).
            self.bars.setOpts(x=[0], height=[0], width=1.0)
