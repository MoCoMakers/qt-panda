...
void serialCommand(String command, STM &stm)
{
    ...
    // ---- Phase 5: push-mode status streaming ----------------------------------
        ...
        stm.setStreamRate(hz);
    }
    // ---- Phase 5: raw ISR tap (RAWD <decim>; 0=off, 1=every tick=25 kHz) ------
    else if (command == "RAWD") {
        int decim = Serial.parseInt();
        stm.setRawDecim(decim);
    }
    // ---- Phase 5: samples/pixel override (SPPX <n>; 0 = auto from LRAT) -------
...
void loop()
{
    ...
    // Push-mode status streaming (STRM <hz>), silent unless enabled
    stm.emitStatusStream();

    // Drain raw ISR-tap blocks (RAWD <decim>), silent unless enabled
    stm.emitPendingRawBlocks();
