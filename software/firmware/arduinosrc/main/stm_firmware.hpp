...
public:
    ...
    // ---- Raw ISR tap (RAWD, Phase 5) ---------------------------------------
    // Every rawDecim-th ISR tick, one (adc i16, z i32, err i32) sample is
    // packed big-endian into a ring of blocks; loop() drains full blocks as
    // binary 'R' frames.  The ISR only ever copies 10 bytes — it never
    ...
    struct RawBlock {
        ...
        uint16_t count = 0;
        uint32_t t0 = 0;                       // time_millis of first sample
        uint8_t  data[RAW_BLOCK_SAMPLES * 10];
    };
    RawBlock rawBlocks[RAW_N_BLOCKS];
    volatile unsigned int rawDecim        = 0; // 0 = off, 1 = every tick
    volatile unsigned int rawDecimCounter = 0;
    volatile unsigned int rawWriteBlock   = 0;
    volatile unsigned int rawWriteIdx     = 0;
    volatile uint32_t     rawDroppedSamples = 0;
    unsigned int          rawReadBlock    = 0;
    uint16_t              rawSeq          = 0;
    ...
    void controlTick() {
        ...
        // Control law (declarative): err = setpoint - measured, so
        //   too little current (tip too far)  -> err > 0 -> z_pos rises -> EXTEND
        //   too much current  (tip too close) -> err < 0 -> z_pos falls -> RETRACT
        // This matches the proven legacy control_current() convention
        // (adc_set_value_log - adc_real_value_log).  The sign was
        // accidentally inverted when the ISR integer PI was written,
        // turning constant-current mode into POSITIVE feedback: engaging
        // CC in contact railed Z to full extension and plunged the tip
        // (bench 2026-07-14, ~950 motor steps of stored strain).
        int err = setpointLog - logTable[abs(adc_val)];
        ...
        if (rawDecim > 0) {
            if (++rawDecimCounter >= rawDecim) {
                rawDecimCounter = 0;
                RawBlock &rb = rawBlocks[rawWriteBlock];
                if (!rb.ready) {
                    if (rawWriteIdx == 0) rb.t0 = millis();
                    uint8_t *p = rb.data + rawWriteIdx * 10;
                    int16_t a = (int16_t)stm_status.adc;   // pre-compensation raw
                    int32_t zz = z_pos;
                    int32_t ee = err;
                    p[0] = (uint8_t)(((uint16_t)a >> 8) & 0xFF);
                    p[1] = (uint8_t)( (uint16_t)a       & 0xFF);
                    p[2] = (uint8_t)(((uint32_t)zz >> 24) & 0xFF);
                    p[3] = (uint8_t)(((uint32_t)zz >> 16) & 0xFF);
                    p[4] = (uint8_t)(((uint32_t)zz >>  8) & 0xFF);
                    p[5] = (uint8_t)( (uint32_t)zz        & 0xFF);
                    p[6] = (uint8_t)(((uint32_t)ee >> 24) & 0xFF);
                    p[7] = (uint8_t)(((uint32_t)ee >> 16) & 0xFF);
                    p[8] = (uint8_t)(((uint32_t)ee >>  8) & 0xFF);
                    p[9] = (uint8_t)( (uint32_t)ee        & 0xFF);
                    if (++rawWriteIdx >= RAW_BLOCK_SAMPLES) {
                        rb.count = RAW_BLOCK_SAMPLES;
                        rb.ready = true;                   // hand to loop()
                        rawWriteIdx = 0;
                        rawWriteBlock = (rawWriteBlock + 1) % RAW_N_BLOCKS;
                    }
                } else {
                    rawDroppedSamples++;                   // loop() is behind
                }
            }
        }
    ...
    // SPPX override: 0 = auto (spp derived from line rate).  Nonzero pins
    // samplesPerPixel directly and the line rate becomes the derived value
    // (rate = 1e6 / (spp * dt * pixels)) — lets slow scans keep raw temporal
    // resolution per pixel instead of silently averaging it away.
    volatile unsigned int sppOverride = 0;
    ...
    void updateStepSizes() {
        ...
        if (sppOverride > 0) new_spp = sppOverride;
    ...
    // =========================================================================
    // Phase 5 — raw ISR tap control (RAWD command)
    // =========================================================================

    void setRawDecim(int decim) {
        noInterrupts();
        rawDecim        = (decim <= 0) ? 0 : (unsigned int)decim;
        rawDecimCounter = 0;
        rawWriteIdx     = 0;
        rawWriteBlock   = 0;
        rawDroppedSamples = 0;
        for (unsigned int i = 0; i < RAW_N_BLOCKS; i++)
            rawBlocks[i].ready = false;
        interrupts();
        rawReadBlock = 0;
        rawSeq       = 0;
    }

    // Drain full raw blocks as binary 'R' frames (loop() context):
    //   'R' | u16 seq | u16 count | u32 t0_millis | u32 dropped_samples
    //   | count*10 bytes (i16 adc, i32 z, i32 err — big-endian, interleaved)
    //   | 0x0A
    void emitPendingRawBlocks() {
        while (rawBlocks[rawReadBlock].ready) {
            RawBlock &rb = rawBlocks[rawReadBlock];
            uint8_t h[13];
            h[0]  = 0x52;                                  // 'R'
            h[1]  = (uint8_t)((rawSeq >> 8) & 0xFF);
            h[2]  = (uint8_t)( rawSeq       & 0xFF);
            h[3]  = (uint8_t)((rb.count >> 8) & 0xFF);
            h[4]  = (uint8_t)( rb.count       & 0xFF);
            h[5]  = (uint8_t)((rb.t0 >> 24) & 0xFF);
            h[6]  = (uint8_t)((rb.t0 >> 16) & 0xFF);
            h[7]  = (uint8_t)((rb.t0 >>  8) & 0xFF);
            h[8]  = (uint8_t)( rb.t0        & 0xFF);
            uint32_t dropped = rawDroppedSamples;
            h[9]  = (uint8_t)((dropped >> 24) & 0xFF);
            h[10] = (uint8_t)((dropped >> 16) & 0xFF);
            h[11] = (uint8_t)((dropped >>  8) & 0xFF);
            h[12] = (uint8_t)( dropped        & 0xFF);
            Serial.write(h, sizeof(h));
            Serial.write(rb.data, (size_t)rb.count * 10);
            Serial.write(0x0A);
            rb.ready = false;                              // free for the ISR
            rawSeq++;
            rawReadBlock = (rawReadBlock + 1) % RAW_N_BLOCKS;
        }
    }

    // =========================================================================
    // Phase 5 — push-mode status streaming (STRM command)
    // The 9 Hz GSTS poll is PC-clocked; this pushes the same status tuple
    // unsolicited from loop() at up to 500 Hz so recordings aren't
    // poll-limited and status keeps flowing during continuous scans.
    // =========================================================================

    unsigned long streamIntervalUs = 0;   // 0 = streaming off
    elapsedMicros streamElapsed;

    void setStreamRate(int hz) {
        if (hz <= 0) { streamIntervalUs = 0; return; }
        if (hz > 500) hz = 500;
        streamIntervalUs = 1000000UL / (unsigned long)hz;
        streamElapsed = 0;
    }

    void emitStatusStream() {
        if (streamIntervalUs == 0) return;
        // Legacy paths (SCST/IV/noise/approach plunge) own the serial line
        // and SPI while blockISRControl is set — stay silent then, so their
        // ASCII replies aren't interleaved with binary frames.
        if (blockISRControl) return;
        if (streamElapsed < streamIntervalUs) return;
        // Drift-free pacing; resync if we fell far behind (USB stall).
        streamElapsed -= streamIntervalUs;
        if (streamElapsed > 10 * streamIntervalUs) streamElapsed = 0;

        uint8_t flags = (stm_status.is_approaching   ? 0x01 : 0)
                      | (stm_status.is_const_current ? 0x02 : 0)
                      | (stm_status.is_scanning      ? 0x04 : 0);
        emitStatusFrame(stm_status.time_millis,
                        (int16_t)stm_status.adc,
                        (uint16_t)stm_status.dac_z,
                        (uint16_t)stm_status.bias,
                        (int32_t)stm_status.steps,
                        flags);
    }
    ...
    void reset() {
        ...
        // Boot to NEUTRAL defaults instead of the DAC rails (operator
        // 2026-07-15): 0 counts = an arbitrary rail (+3 V bias / -5 V X,Y).
        // Bias 0 V and centered X/Y are the safe no-drive starting state.
        // Z is DELIBERATELY LEFT RETRACTED (dac_z.reset) — it is the crash
        // axis; the tip may be near the surface at boot, so never auto-
        // extend it to mid-range.
        set_dac_bias(32768);   // 0 V bias
        set_dac_x(32768);      // centered
        set_dac_y(32768);
    ...
    bool approach() {
        ...
        if (stepper_motor.get_total_steps() < approach_config.max_steps) {
            ...
            stm_status.is_approaching = false;
            // De-energize the coils on this exit path too — leaving holding
            // current flowing after a failed approach is heat + magnetic
            // noise at the tip (bench issue 2026-07-09; the hit path already
            // disables via stepper_motor.disable()).
            stepper_motor.disable();
            blockISRControl = savedBlock;
            return false;
        }
