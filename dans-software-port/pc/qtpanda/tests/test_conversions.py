...
def test_xy_span_matches_calibration(sc):
    c = sc._cal
    nm = 30.0
    expected = round(nm / c.piezo_x_nm_per_v / c.dac_x_v_per_lsb)
    assert sc._nm_to_xy_lsb_span(nm) == expected
    # Sanity: 30 nm at defaults stays within the 16-bit DAC span.
    assert 0 < sc._nm_to_xy_lsb_span(30.0) < 65535
